...
///
/// Redefined here because the redefinition is not available in all supported
/// Windows Target versions.
...
// Retrieves any DPI transformation applied to the window with
// |window_handle|.
float GetDpiForMonitor(HMONITOR monitor) {
  auto get_dpi_for_monitor =
      reinterpret_cast<HRESULT(WINAPI*)(HMONITOR, MONITOR_DPI_TYPE, UINT*,
                                        UINT*)>(
          GetProcAddress(GetModuleHandle(L"Shcore.dll"), "GetDpiForMonitor"));
  if (get_dpi_for_monitor) {
    UINT dpi_x = 0, dpi_y = 0;
    if (SUCCEEDED(
            get_dpi_for_monitor(monitor, MDT_EFFECTIVE_DPI, &dpi_x, &dpi_y))) {
      return static_cast<float>(dpi_x);
    }
  }
  return static_cast<float>(USER_DEFAULT_SCREEN_DPI);
}
...
// Count of active Win32Window instances; used to unregister the window class.
